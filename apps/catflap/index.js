...
const rpSureflapAPIStatusOptions = {
  uri: "https://app.api.surehub.io/api/device/",
  qs: {
    with: "status",
  },
  headers: {
...
alexaApp.pre = async function (request, response, type) {
  ...
  sureFlapPetPositionData = (await rp(rpSureflapAPIOptions)).data;
  ...
  sureflapDeviceData.forEach((flap) => {
    if (flap.id === 66454 || flap.id === 294131) return; // don't need parents
    ...
    if (flap.name == "Utility") {
      flapIcon = "fa-home";
    }
    if (flap.name == "Garden Room") {
      flapIcon = "fa-couch";
    }
    if (flap.name == "Garage Door") {
      flapIcon = "fa-warehouse";
    }
    if (flap.name == "Landing") {
...
function getMatchedLocation(request) {
  logger.info("getMatchedLocation");
